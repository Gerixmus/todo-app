...
{
    fun completedDate(): LocalDate? = if (completedDateString == null) null else
        LocalDate.parse(completedDateString, dateFormatter)
    fun dueTime(): LocalTime? = if (dueTimeString == null) null else
    ...
    fun imageColour(context: Context): Int = if(isCompleted()) purple(context) else black(context)
